...
Runs the prompt in `src/lib/prompts/translation-feedback.ts` against OpenAI. The yaml does not duplicate the system prompt: `prompt.ts` calls `buildPrompt` with each test's `vars` and sets the JSON schema.
